...
import { createDocument, createWorkspace, createWorkspaceDocument, documentsForWorkspace, fileNameFromPath, isDirty, markDocumentLoaded, markSaved, mergeDocuments, mergeWorkspaces, relativePathFromRoot, updateMarkdown, workspaceTreeNodes } from './document';
...
describe('document state', () => {
  ...
  it('creates and loads workspace documents only when their contents are needed', () => {
    const workspace = createWorkspace('C:\\notes');
    const unloaded = createWorkspaceDocument('C:\\notes\\today.md', workspace);
    ...
    expect(markDocumentLoaded([unloaded], unloaded.path, '# Today')[0]).toMatchObject({
      markdown: '# Today',
      savedMarkdown: '# Today',
      loaded: true,
    });
